...
if (isset($_GET['acao'])) {
    ...
    if ($acao == 'bulk_update' && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        elseif ($bulk_action == 'excluir') {
            foreach ($produto_ids as $id) {
                ...
                if ($res_img && $prod_dados = $res_img->fetch_assoc()) {
                    if (!empty($prod_dados['imagem'])) {
                        $caminho = '../assets/uploads/' . $prod_dados['imagem'];
                        if (file_exists($caminho)) unlink($caminho);
                    }
                }


                $stmt_gal = $conexao->prepare("SELECT caminho_imagem FROM produto_imagens WHERE produto_id = ?");
                $stmt_gal->bind_param("i", $id);
                $stmt_gal->execute();
                $res_gal = $stmt_gal->get_result();
                while ($img = $res_gal->fetch_assoc()) {
                    $caminho_gal = '../assets/uploads/' . $img['caminho_imagem'];
                    if (file_exists($caminho_gal)) unlink($caminho_gal);
                }


                $stmt_del = $conexao->prepare("DELETE FROM produtos WHERE id = ?");
                $stmt_del->bind_param("i", $id);
                $stmt_del->execute();
            }
            
 
            header("Location: gerenciar_produtos.php?sucesso=6");
            exit();
        }
    ...
    elseif ($acao == 'excluir' && isset($_GET['id'])) {
        ...
        $stmt->execute();
        $res = $stmt->get_result();
        if ($res && $produto = $res->fetch_assoc()) {
            if (!empty($produto['imagem'])) {
                $caminho_arquivo = '../assets/uploads/' . $produto['imagem'];
        ...
        $stmt_delete = $conexao->prepare("DELETE FROM produtos WHERE id = ?");
        $stmt_delete->bind_param("i", $id);

        if ($stmt_delete->execute()) {
            header("Location: gerenciar_produtos.php?sucesso=5"); 
        } else {
            header("Location: gerenciar_produtos.php?erro=excluir");
        }
        exit();
    }
}
